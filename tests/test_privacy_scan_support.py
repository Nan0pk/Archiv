...
def test_a_word_that_merely_resembles_the_account_name_is_not_a_match(tmp_path: Path) -> None:
    ...
    with the account name -- e.g. hardware named the same as this machine's user.
    Matching the bare username token (the old design) could not tell these apart;
    matching the full home-directory path can.
    """

    _init_repo(tmp_path)
    (tmp_path / "hardware.md").write_text("Tested on an HP Victus laptop.", encoding="utf-8")
    subprocess.run(["git", "add", "hardware.md"], cwd=tmp_path, check=True)
    subprocess.run(["git", "commit", "-q", "-m", "init"], cwd=tmp_path, check=True)

    hits = files_containing(tmp_path, "/home/victus")
