...
def test_fedora_installer_local_source_and_upgrade(tmp_path: Path) -> None:
    ...
    # Simulate the prior 0.1.0a3 release already installed.
    old_version_dir = prefix / "versions" / "0.1.0a3"
    ...
    assert ver_res.stdout.strip() == "0.1.0a4"
    ...
    assert current_symlink.resolve() == (prefix / "versions" / "0.1.0a4").resolve()
